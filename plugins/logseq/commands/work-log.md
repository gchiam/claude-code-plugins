...
3. **Read today's journal**: Call `mcp__graphthulhu__get_page` with today's journal page name. Note which of the four section headings already exist: `## In Progress`, `## Done`, `## Blocked`, `## Next`.
...
5. **Append each item**:
   ...
   - If the section heading **does not exist**: write a single `upsert_blocks` call with the heading as parent and the item as its child (see block-examples.md for the exact JSON shape)
